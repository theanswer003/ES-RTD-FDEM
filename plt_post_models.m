...
set(gcf, 'Position', [0.1*screenWidth 0.1*screenHeight ...
    0.8*screenWidth 0.6*screenHeight])
subplot(321)
imagesc([0, nx]*0.1, [0, nz]*0.1, post_EC_mu'); 
...
subplot(322)
imagesc([0, nx]*0.1, [0, nz]*0.1, post_MS_mu'); 
xticks([]); ylabel('Depth (m)')
title('True MS')
...
subplot(323)
imagesc([0, nx]*0.1, [0, nz]*0.1, post_EC_std'); 
xticks([]);  ylabel('Depth (m)')
title('Prior mean of EC')
% caxis([minEC, maxEC]); 
c3 = colorbar ; ylabel(c3, 'S/m');
dim = [.09+0.01 .56 .1 .1];
...
subplot(324)
imagesc([0, nx]*0.1, [0, nz]*0.1, post_MS_std'); 
xticks([]); ylabel('Depth (m)')
title('Prior mean of MC')
% caxis([minMS, maxMS]); 
c4 = colorbar ; ylabel(c4, 'SI');
dim = [.53+0.01 .56 .1 .1];
...
subplot(325)
imagesc([0, nx]*0.1, [0, nz]*0.1, abs(post_EC_mu'-EC_true')); 
xlabel('Distance (m)'); ylabel('Depth (m)'); xticks(5:5:35);
title('Absolute error of EC')
% caxis([minEC, maxEC]); 
c5 = colorbar ; ylabel(c5, 'S/m');
dim = [.09+0.01 .26 .1 .1];
h = annotation('textbox', dim, 'String', '(e)', ...
    'fontsize', 14, 'fontweight', 'bold','FitBoxToText','on');
h.LineStyle = 'none';

subplot(326)
imagesc([0, nx]*0.1, [0, nz]*0.1, abs(post_MS_mu'-MS_true')); 
xlabel('Distance (m)'); ylabel('Depth (m)'); xticks(5:5:35);
title('Absolute error of MC')
% caxis([minMS, maxMS]); 
c6 = colorbar ; ylabel(c6, 'SI');
dim = [.53+0.01 .26 .1 .1];
h = annotation('textbox', dim, 'String', '(f)', ...
    'fontsize', 14, 'fontweight', 'bold','FitBoxToText','on');
h.LineStyle = 'none';
